...
#### setup (set custom data path for nltk)
nltk.data.path.append("../resources/nltk_data")
# training serialization file (del for regenerate it)
tokenizer_train_file_path = "data/trained_tokenizer.pickle"
...
class SentenceTokenizer():
    ...
    def punct_features(self, tokens, i):
        return {
            'next-word-capitalized':
                 (i < len(tokens) - 1) and tokens[i + 1][0].isupper(),
    ...
    def punct_features2(self, tokens, i):
        return {
            'next-word-capitalized':
                 (i < len(tokens) - 1) and tokens[i + 1][0][0].isupper(),
            'prevword': tokens[i - 1][0].lower(),
            'punct': tokens[i][0],
            'prev-word-is-one-char': len(tokens[i - 1][0]) == 1}

    # The constructor builds a classifier using training data
    # Naive Bayes is used for fast training
    # The entire dataset is used for training
    def __init__(self):
        self.tokenizer = ModifiedWPTokenizer()
        if os.path.isfile(tokenizer_train_file_path):
            #load it
            f = open(tokenizer_train_file_path, 'rb')
            self.classifier = pickle.load(f)
            f.close()
        else:  # if file not exist, train and store the classifier
            tokens = []
            boundaries = set()
            offset = 0
            for sent in nltk.corpus.cess_esp.sents():
                tokens.extend(sent)
                offset += len(sent)
            ...
            featuresets = [
                (self.punct_features(tokens, i), (i in boundaries))
            ...
            #save it
            f = open(tokenizer_train_file_path, 'wb')
        ...
        for i, word in enumerate(words):
            if (word[0] in '.?!' and
                self.classifier.classify(
                    self.punct_features2(words, i)) is True):
        ...
        for sent in self.classify_segment_sentences(word_tuples):
            ...
            for i, tup in enumerate(sent):
                if (tup[0][0] in string.punctuation and not tup[0][0] in '.?!'):
                    ...
                    # penultimate end of the sentence
                    #- break off the punctuation
                    sentence.append(tok + tup[0])
                    tok = ""
                else:
